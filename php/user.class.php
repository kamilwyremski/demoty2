...
class user {
	...
	public static function resetPasswordNew($code){
		global $db;
	...
	public static function resetPasswordNewCheck(int $user_id,$data,$code){
		global $db, $settings;
	...
	public function checkCompleteData($code){
		global $db;
		$sth = $db->prepare('SELECT 1 FROM '._DB_PREFIX_.'user WHERE ISNULL(username) AND active=1 AND activation_code=:code LIMIT 1');
		$sth->bindValue(':code', $code, PDO::PARAM_STR);
		$sth->execute();
		if($sth->fetchColumn()){
			return true;
	...
	public function completeData($code,$data){
		...
		if(isset($error)){
			...
			$sth = $db->prepare('UPDATE '._DB_PREFIX_.'user SET username=:username WHERE active=1 AND activation_code=:code AND ISNULL(username) LIMIT 1');
			$sth->bindValue(':username', $data['username'], PDO::PARAM_STR);
			$sth->bindValue(':code', $code, PDO::PARAM_STR);
			$sth->execute();
	...
	public function getAllData(){
		...
		$sth = $db->prepare('SELECT count(1) FROM '._DB_PREFIX_.'picture WHERE user_id=:user_id');
		$sth->bindValue(':user_id', $this->id, PDO::PARAM_INT);
		$sth->execute();
		$this->user_data['number_files'] = $sth->fetchColumn();

		$sth = $db->prepare('SELECT count(1) FROM '._DB_PREFIX_.'picture WHERE user_id=:user_id AND waiting_room = 1');
		$sth->bindValue(':user_id', $this->id, PDO::PARAM_INT);
		$sth->execute();
	...
	public function changePassword($data){
		...
		if($this->checkPassword($data['old_password'], $this->password)){
				...
				$sth->bindValue(':password', static::createPassword($data['new_password']), PDO::PARAM_STR);
				$sth->execute();
				return ['status'=>true];
			}else{
	...
	public function loginByEmail($email,$source=''){
		...
		if(checkEmailBlackList($email) or checkIpBlackList(getClientIp())){
			...
			if($user_data!=''){
				...
				$sth->bindValue(':password', static::createPassword($password), PDO::PARAM_STR);
				$sth->bindValue(':activation_code', $activation_code, PDO::PARAM_STR);
	...
	public static function getProfile($username){
		...
		(SELECT count(1) FROM '._DB_PREFIX_.'picture WHERE user_id=u.id) AS number_files,
		(SELECT count(1) FROM '._DB_PREFIX_.'picture WHERE user_id=u.id AND waiting_room = 1) AS number_files_waiting_room,
	...
	public static function remove(int $id){
		global $db;
		$sth = $db->prepare('SELECT id FROM '._DB_PREFIX_.'picture WHERE user_id=:user_id');
		$sth->bindValue(':user_id', $id, PDO::PARAM_INT);
		$sth->execute();
		foreach($sth as $row){;
			picture::remove($row['id']);
		}
