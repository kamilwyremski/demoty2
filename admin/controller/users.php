...
if($admin->is_logged()){
	...
	$sth = $db->prepare('SELECT SQL_CALC_FOUND_ROWS u.*,
	(SELECT COUNT(1) FROM '._DB_PREFIX_.'picture WHERE user_id=u.id) AS amount_pictures,
	(SELECT COUNT(1) FROM '._DB_PREFIX_.'picture WHERE user_id=u.id AND waiting_room=0) AS amount_pictures_main_page,
	...
	$sth->bindValue(':limit_from', paginationPageFrom('users'), PDO::PARAM_INT);
	$sth->bindValue(':limit_to', $limit, PDO::PARAM_INT);
	foreach($bind_values as $key => $value){
		$sth->bindValue(':'.$key, $value, PDO::PARAM_STR);
	}
	$sth->execute();
	while ($row = $sth->fetch(PDO::FETCH_ASSOC)){	$users[] = $row; }
