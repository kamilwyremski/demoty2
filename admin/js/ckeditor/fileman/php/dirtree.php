...
function getPicturesNumber($path, $type){
  $files = 0;
  $dirs = 0;
...
function GetDirs($path, $type){
  ...
  foreach ($files as $f){
    ...
    if(!is_dir(fixPath($fullPath)) || $f == '.' || $f == '..')
      continue;
    $tmp = getPicturesNumber(fixPath($fullPath), $type);
...
echo "[\n";
$tmp = getPicturesNumber(fixPath(getPicturesPath()), $type);
echo '{"p":"'.  mb_ereg_replace('"', '\\"', getPicturesPath()).'","f":"'.$tmp['files'].'","d":"'.$tmp['dirs'].'"}';
GetDirs(getPicturesPath(), $type);
echo "\n]";
?>