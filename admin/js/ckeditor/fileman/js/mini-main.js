...
RoxyUtils.getPicturename = function(path){
  var ret = path;
  path = RoxyUtils.FixPath(path);
  if(path.indexOf('/') > -1){
...
RoxyUtils.getPictureExt = function(path){
  var ret = '';
  path = RoxyUtils.getPicturename(path);
  if(path.indexOf('.') > -1){
    ret = path.substring(path.lastIndexOf('.') + 1);
  }
...
RoxyUtils.getPictureIcon = function(path){
  ret = 'images/filetypes/unknown.png';//'images/filetypes/file_extension_' + RoxyUtils.getPictureExt(path).toLowerCase() + '.png';
  if(fileTypeIcons[RoxyUtils.getPictureExt(path).toLowerCase()]){
    ret = 'images/filetypes/' + fileTypeIcons[RoxyUtils.getPictureExt(path).toLowerCase()];
  }

  return ret;
};
RoxyUtils.getPicturesize = function(path){
  var ret = 0;
  $.ajax({
      url: path,
      type: 'HEAD',
...
RoxyUtils.getPictureType = function(path){
  var ret = RoxyUtils.getPictureExt(path).toLowerCase();
...
RoxyUtils.IsImage = function(path){
  var ret = false;
  if(RoxyUtils.getPictureType(path) == 'image')
    ret = true;
...
function File(filePath, fileSize, modTime, w, h){
  ...
  this.type = RoxyUtils.getPictureType(filePath);
  this.name = RoxyUtils.getPicturename(filePath);
  this.ext = RoxyUtils.getPictureExt(filePath);
  this.path = RoxyUtils.GetPath(filePath);
  this.icon = RoxyUtils.getPictureIcon(filePath);
  this.bigIcon = this.icon.replace('filetypes', 'filetypes/big');
  this.image = filePath;
  this.size = (fileSize?fileSize:RoxyUtils.getPicturesize(filePath));
  this.time = modTime;
  this.width = (w? w: 0);
  this.height = (h? h: 0);
  this.Show = function(){
    ...
    $.ajax({
        ...
        success: function(data){
            if(data.res.toLowerCase() == 'ok'){
              ...
              $('li[data-path="'+item.fullPath+'"] .icon').attr('src', RoxyUtils.getPictureIcon(newName));
...
function Directory(fullPath, numDirs, numFiles){
  ...
  this.name = RoxyUtils.getPicturename(fullPath);
  if(!this.name)
    this.name = 'My files';
  ...
  this.Update = function(newPath){
    ...
    if(newPath){
      ...
      this.name = RoxyUtils.getPicturename(newPath);
      if(!this.name)
        this.name = 'My files';
...
function selectFile(item){
  ...
  var html = RoxyUtils.getPicturename($(item).attr('data-path'));
...
function getPreselectedFile(){
  ...
  if(!filePath){
    switch(getPicturemanIntegration()){
      case 'ckeditor':
        try{
...
$(function(){
  ...
  if(getPicturemanIntegration() == 'tinymce3'){
    try {
...
function getPicturemanIntegration(){
